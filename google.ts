...
// in the environment. Tokens live in google_tokens, keyed on (owner_id, slot)
// so one user can connect a "personal" and a "work" calendar separately;
...
// state carries the CSRF token AND the slot, "<uuid>.<slot>". The cookie
// stores the same full string, so parseState below can split it back apart
// after verifying the round trip matches.
export function authUrl(origin: string, state: string, slot: CalendarSlot): string {
...
export function parseState(state: string): { token: string; slot: CalendarSlot } | null {
  const i = state.lastIndexOf(".");
  if (i === -1) return null;
  const token = state.slice(0, i);
  const slot = state.slice(i + 1);
  if (slot !== "personal" && slot !== "work") return null;
  return { token, slot };
}
...
// Returns a live access token for the user's given slot, or null if that
// slot isn't connected.
export async function getAccessToken(
  supabase: SupabaseClient,
  userId: string,
  slot: CalendarSlot = "personal"
): Promise<string | null> {
...
// The user's primary calendar for the next N days. Pass `source` to tag
// every returned event with which slot it came from (useful once you're
// merging personal + work events for the brief).
...
// Writes always go to the "personal" slot's calendar (the app has never
// created events on a work calendar, and shouldn't without an explicit ask).
